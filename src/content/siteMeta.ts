function readEnv(key: "VITE_SITE_URL" | "VITE_APP_URL", fallback: string): string {
  const fromProcess = typeof process !== "undefined" ? process.env[key]?.trim() : undefined;
  if (fromProcess) return fromProcess;

  try {
    const fromImport = import.meta.env[key]?.trim();
    if (fromImport) return fromImport;
  } catch {
    /* import.meta unavailable outside Vite client bundle */
  }

  return fallback;
}

export const SITE_URL = readEnv("VITE_SITE_URL", "https://12vsim.com");
export const APP_URL = readEnv("VITE_APP_URL", "https://app.12vsim.com");
